...
ENERGINET_DEFAULT_THRESHOLDS: Dict[int, float] = {2: 400.0, 3: 600.0, 4: 2400.0}
...
def _validate_input_tables(data: Dict[str, pd.DataFrame]) -> Tuple[np.ndarray, List[str]]:
    ...
    ref_sheet = REQUIRED_SHEETS[0]
    ref_df = data[ref_sheet]
    ref_freq = _frequency_vector(ref_df, ref_sheet)
    ref_cases = _sheet_case_columns(ref_df)
    if not ref_cases:
    ...
    for name in REQUIRED_SHEETS[1:]:
        ...
        if freq.shape != ref_freq.shape or not np.allclose(freq, ref_freq, rtol=0.0, atol=1e-9):
            raise ValueError("Frequency column mismatch across required sheets.")
        cases = _sheet_case_columns(df)
        if cases != ref_cases:
            raise ValueError("Case columns mismatch across required sheets.")
    return ref_freq, ref_cases


def _extract_case_arrays(df: pd.DataFrame, cases: Sequence[str], sheet_name: str) -> Dict[str, np.ndarray]:
    out: Dict[str, np.ndarray] = {}
    for case in cases:
        if case not in df.columns:
            raise ValueError(f"Case '{case}' not found in sheet '{sheet_name}'.")
        out[str(case)] = pd.to_numeric(df[case], errors="coerce").to_numpy(dtype=float)
    return out
...
def _compute_iec_vertices(
    ...
    for n in range(2, n_env + 1):
        ...
        if len(unique_points) < 3:
            selected_cases = sorted(case_points.keys())
        else:
            hull_pts = _convex_hull(unique_points)
            selected: List[str] = []
            for hp in hull_pts:
                selected.extend(point_to_cases.get(hp, []))
...
def build_preselection_payload(
    ...
    freq, all_case_ids = _validate_input_tables(data)
    ...
    r1_map = _extract_case_arrays(data["R1"], chosen_cases, "R1")
    x1_map = _extract_case_arrays(data["X1"], chosen_cases, "X1")
    ...
    for f1 in fundamentals_hz:
        ...
        iec = _compute_iec_vertices(freq, r1_map, x1_map, chosen_cases, f1_val)
        by_f1[f1_key] = {
            "energinet_metrics": dict(energinet["energinet_metrics"]),
            "band_sample_counts": dict(energinet["band_sample_counts"]),
            "iec_case_ids": list(iec["iec_case_ids"]),
            "iec_first_harmonic": dict(iec["iec_first_harmonic"]),
            "n_env": int(iec["n_env"]),
        }
