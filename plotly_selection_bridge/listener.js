...
function getPreselectionForBase(ctx) {
  ...
  const baseKey = String(Math.round(Number(ctx.state.baseFrequencyHz || 50)));
  const candidate = byF1[baseKey] || byF1["50"] || byF1["60"] || null;
...
function computeEnerginetCandidates(ctx) {
  ...
  for (const item of ctx.casesMeta) {
    ...
    const f2 = Number.isFinite(z2) && Number.isFinite(t2) && t2 > 0 && z2 > t2;
    const f3 = Number.isFinite(z3) && Number.isFinite(t3) && t3 > 0 && z3 > t3;
    const f4 = Number.isFinite(z4) && Number.isFinite(t4) && t4 > 0 && z4 > t4;
    if (f2 || f3 || f4) out.add(cid);
  }
  return out;
}
...
function computeIecCandidates(ctx) {
  ...
  const baseData = getPreselectionForBase(ctx);
  if (!baseData) return out;
  const ids = Array.isArray(baseData.iec_case_ids) ? baseData.iec_case_ids : [];
  for (const cidRaw of ids) {
    const cid = String(cidRaw || "");
    if (!cid) continue;
    if (ctx.caseById.has(cid)) out.add(cid);
  }
  return out;
}
...
function syncAxisResetBaseline(gd, axisName, rangeVals) {
  if (!gd || !axisName || !Array.isArray(rangeVals) || rangeVals.length !== 2) return;
  const a0 = Number(rangeVals[0]);
  const a1 = Number(rangeVals[1]);
  if (!Number.isFinite(a0) || !Number.isFinite(a1) || a1 <= a0) return;
  try {
    if (!gd.layout || typeof gd.layout !== "object") gd.layout = {};
    if (!gd.layout[axisName] || typeof gd.layout[axisName] !== "object") gd.layout[axisName] = {};
    gd.layout[axisName].range = [a0, a1];
    gd.layout[axisName].autorange = false;
  } catch (e) {}
  try {
    const ax = gd._fullLayout && gd._fullLayout[axisName] ? gd._fullLayout[axisName] : null;
    if (!ax) return;
    ax.range = [a0, a1];
    ax.autorange = false;
    ax._rangeInitial0 = a0;
    ax._rangeInitial1 = a1;
    ax._rangeInitial = [a0, a1];
...
function applyLineBaseFrequency(ctx, gd) {
  ...
  const prevBaseHz = Number(gd && gd.__fsBaseFreqHz != null ? gd.__fsBaseFreqHz : (latestArgs && latestArgs.f_base != null ? latestArgs.f_base : 50));
  ...
  for (let ti = 0; ti < gd.data.length; ti++) {
    ...
    const extracted = extractFrequencyHzArrayFromLineTrace(tr, prevBaseHz);
    if (!(extracted && extracted.finiteCount > 0) || !Array.isArray(extracted.hz) || !extracted.hz.length) {
      continue;
    }
    const xVals = [];
    for (let i = 0; i < extracted.hz.length; i++) {
      const hz = Number(extracted.hz[i]);
      if (!Number.isFinite(hz)) {
        xVals.push(null);
      } else {
  ...
  try {
    ...
    if (Number.isFinite(minHz) && Number.isFinite(maxHz) && maxHz > minHz) {
      fullRange = [minHz / baseHz, maxHz / baseHz];
      Plotly.relayout(gd, { "xaxis.range": fullRange });
      // Keep Plotly modebar Reset axes baseline in sync with current base frequency.
      syncAxisResetBaseline(gd, "xaxis", fullRange);
    }
...
function bindScatterHandlers(ctx, gd, plotIndex) {
  ...
  const clickHandler = (evt) => {
    ...
    try {
      ...
      const selectedNow = st.selectedCases instanceof Set ? st.selectedCases.has(caseId) : false;
      const selectedByMethodNow = isCaseSelectedByActiveMethod(st, caseId);

      if (selectedNow) {
        if (manual.has(caseId)) manual.delete(caseId);
        if (selectedByMethodNow) excluded.add(caseId);
      } else {
        manual.add(caseId);
        excluded.delete(caseId);
      }
